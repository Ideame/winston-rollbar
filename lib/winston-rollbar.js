...
var Rollbar = exports.Rollbar = function (options) {
    ...
    if (!options.rollbarAccessToken) {
        throw "winston-rollbar requires 'rollbarAccessToken' property ";
    }
...
Rollbar.prototype.log = function (level, msg, meta, callback) {
    ...
    if (this.metadataAsRequest) {
        if (req.socket) { req = meta; }
    } else if (meta && meta.req) {
        req = meta.req;
    }

    if (/error/i.test(level) && (msg instanceof Error || meta instanceof Error)) {
        var error = msg instanceof Error ? msg : meta;

        rollbar.handleError(error, req, function (err) {
